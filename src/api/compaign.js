...
* 获取推广计划
*/
export function getCampaign_old(subway_token){
    return new Promise((resolve, reject) => {
        QN.top.invoke({
            query: {
                method: 'taobao.simba.campaigns.get', // TOP 接口名称
            ...
            if(compaingn.length > 0){
                getCompaignReport(subway_token,compaingn).then((data) => {
                    var newData = _.sortBy(data, [function(o) { return o.campaign_id;}]);
                    resolve(newData);
                }).catch(err => {
                    reject(err);
                });
...
* 获取推广计划报表数据
*/
 function getCompaignReport_old(subway_token,campaign){
    var data= [];
    return new Promise((resolve, reject) => {
