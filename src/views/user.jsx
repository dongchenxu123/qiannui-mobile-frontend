...
class User extends Component{
          ...
          this.setState({subway_token : result});
         	ProfileReport(this.state.subway_token).then((result) 		=> {
          	this.setState({
                    ...
                    threedaysago: result.threedaysago
                })
 ...
 submitDate () {
 		var start_date= this.state.start_date == '' ? '2016-01-05' : this.state.start_date;
 		var end_date= this.state.end_date == '' ? '2016-01-06' : this.state.end_date;
		ProfileReport(this.state.subway_token, start_date, end_date).then((result)=> {
             	this.setState({
             		dateAlldays: result.alldays,
             		checked: true
             	})
            }, (error) => {
                Modal.alert(JSON.stringify(error));
      ...
      return (
        <View>
          <View style={styles.amoutList}>
          	<View style={styles.amoutitemArrow}>
		          ...
		          <Text style={styles.title}>
		              ￥{this.state.WuxianData === '' ? 0 : this.state.WuxianData}
	          	</Text>
          	</View>
          	<View style={styles.amoutTextList}>
               ...
               <Text style={styles.title}>
                  ￥{
                      this.state.yesterday.length === 0 ? 0 :
          ...
          <TabSlider navTop={true} width={750} style={styles.barStyle} active={this.state.active} customBar={true} index={this.state.index} onChange={this.sliderChange.bind(this)}>
            ...
            <Pane style={{width:750}} renderTitleCustom={this.siwtchTitleCustom.bind(self,3,this.state.active)}>
            <View style={styles.dateList}>
               			<Button style={styles.amoutitemArrow} onPress={this.startDate}>
               			{
            					this.state.start_date === '' ? '开始时间' : this.state.start_date
            				}
               			</Button>
               			<Button style={styles.amoutitemArrow} onPress={this.endDate}>{
            					this.state.end_date === '' ? '结束时间' : this.state.end_date
            				}</Button>
               			<Button style={styles.amoutTextList} onPress={this.submitDate.bind(this)} checked={this.state.checked}>确定</Button>
               </View>
		           { this.state.checked === false ? <Text style={styles.textCenter
		            }>请您选择日期</Text> : <ListViewCommon amount={this.state.dateAlldays} style={styles.listContainer}/>
		             }
            </Pane>
        </TabSlider>
